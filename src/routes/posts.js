...
router.get('/', optionalAuth, async (req, res) => {
  const { page = 1, limit = 15, search = '', sort = 'latest' } = req.query;
  ...
  try {
    ...
    let orderClause = sort === 'hot' ? 'ORDER BY p.like_count DESC, p.comment_count DESC' : 'ORDER BY p.updated_at DESC';

    const countQuery = `SELECT COUNT(*) FROM posts p ${whereClause}`;
    const dataQuery = `
      SELECT p.id, p.title, LEFT(p.content, 200) as excerpt, p.view_count, p.like_count, p.comment_count, 
...
router.get('/:id', optionalAuth, async (req, res) => {
  try {
    ...
    let liked = false;
    if (req.user) {
      const likeCheck = await pool.query('SELECT id FROM post_likes WHERE post_id=$1 AND user_id=$2', [req.params.id, req.user.id]);
...
router.post('/:id/like', authenticate, async (req, res) => {
  try {
    const existing = await pool.query('SELECT id FROM post_likes WHERE post_id=$1 AND user_id=$2', [req.params.id, req.user.id]);
